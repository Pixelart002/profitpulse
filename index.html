...
<body class="bg-gray-900 text-white font-sans">
  ...
  <script>
    ...
    async function loadUserBlogs() {
      const res = await window.gapi.client.blogger.blogs.listByUser({ userId: 'self' });
    ...
    async function publishToBlogger() {
      ...
      try {
        await window.gapi.client.blogger.posts.insert({ blogId, resource: post });
